...
/**
 * @brief HTanH hyperbolic tangent non-linearity @f$
 *         y = \frac{\exp(2x) - 1}{\exp(2x) + 1}
 *     @f$, popular in auto-encoders.
 *
 * Note that the gradient vanishes as the values move away from 0.
 ...
 */
template <typename Dtype>
class HTanHLayer : public NeuronLayer<Dtype> {
 public:
  explicit HTanHLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "HTanH"; }
